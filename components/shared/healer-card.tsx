...
export function HealerCard({ name, role, image, link }: Healer) {
  return (
    <Link
      href={link}
      className="group overflow-hidden rounded-3xl bg-white border border-primary/20 transition-all duration-500 hover:shadow-xl hover:border-primary/40 hover:-translate-y-2 relative"
    >
      <div className="absolute inset-0 bg-gradient-to-br from-[#f7faf6] to-white opacity-70 pointer-events-none"></div>
      <div className="absolute inset-0 rounded-3xl border-2 border-transparent group-hover:border-primary/30 transition-all duration-500 pointer-events-none"></div>
      ...
      <div className="p-6 text-center relative">
        ...
        <div className="mt-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <button className="text-xs uppercase tracking-wider text-primary border border-primary/40 rounded-full px-4 py-2 hover:bg-primary hover:text-white transition-colors duration-300 min-h-[44px]">
            View Profile
          </button>
        </div>
      </div>
